...
def prune_messages(
    ...
    if tool_max_chars is None:
        tool_max_chars = budget.jit_tool_output_chars
    system_max_chars = budget.session_summary_chars
    ...
    for i, msg in enumerate(messages):
        ...
        pruned_msg = _prune_single_message(msg, tool_max_chars, system_max_chars)
        pruned.append(pruned_msg)

    return pruned


def _prune_single_message(msg: dict[str, Any], max_chars: int, system_max_chars: int) -> dict[str, Any]:
    """修剪单条消息中的大型工具输出"""
    tool_calls = msg.get("tool_calls")
    if not tool_calls:
        content = msg.get("content", "")
        if msg.get("role") == "system" and len(content) > system_max_chars:
            return {
                **msg,
                "content": content[:system_max_chars] + "\n...[已修剪]",
            }
        return msg
